...
// ─── Dev services bootstrap ──────────────────────────────────────────────────

async function bootstrapDevServices(): Promise<string> {
  log("BOOTSTRAPPING dev services on main branch...");
  const { code, stdout } = await runClaudeTask(
    buildBootstrapPrompt(),
    { taskName: "get-shit-done: bootstrap dev services" },
  );
  writeTaskLog("bootstrap", "main", stdout);
  if (code !== 0) {
    log(
      "WARN: Dev services bootstrap failed — forges will skip page inspection if no server found",
    );
    return "";
  }
  log("Dev services bootstrapped on main branch");
  return stdout;
}
...
async function forgeGroup(
  ...
  const results = await Promise.allSettled(
    group.map((t) => forgeTicket(t, repos, devServerInfo)),
  );
...
async function processGroup(
  ...
  const devServerInfo = hasFrontend ? await bootstrapDevServices() : "";

  const forgeResults = await forgeGroup(group, repos, devServerInfo);
