...
describe("mergeAndVerify", () => {
  ...
  it("returns all succeeded when merge, verify, and PR pass", async () => {
    ...
    const result = await mergeAndVerify(
      successForges, ["EC-1", "EC-2"], ["/repo"], true,
  ...
  it("returns all failed when no successful forges", async () => {
    const forges: ForgeResult[] = [
      { ticketKey: "EC-1", status: "failed", worktreePath: "" },
    ];
    const runner = makeRunner([]);
    const { log } = collectLogs();

    const result = await mergeAndVerify(
      forges, ["EC-1"], ["/repo"], true,
      runner, makeDevServers(), makeJira(), makeTracker(), log,
    );
  ...
  it("returns all failed when merge fails", async () => {
    const runner = makeRunner([
  ...
  it("separates failed forges from successful ones in result", async () => {
  ...
  it("marks successful tickets in tracker", async () => {
    const runner = makeRunner([
    ...
    await mergeAndVerify(
      successForges, ["EC-1", "EC-2"], ["/repo"], true,
      runner, makeDevServers(), makeJira(), tracker, log,
    );

    assert.deepEqual(tracker.marked, ["EC-1", "EC-2"]);
  });

  it("moves tickets to In Progress via jira", async () => {
    ...
    await mergeAndVerify(
      successForges, ["EC-1", "EC-2"], ["/repo"], true,
      runner, makeDevServers(), jira, makeTracker(), log,
    );

    assert.deepEqual(movedTickets, ["EC-1", "EC-2"]);
  });

  it("skips verify when hasFrontend is false", async () => {
    let runCallCount = 0;
    const runner = {
      run: async () => {
        runCallCount++;
    ...
    await mergeAndVerify(
      [successForges[0]], ["EC-1"], ["/repo"], false,
      runner, makeDevServers(), makeJira(), makeTracker(), log,
    );
  ...
  it("calls restartOnBranch when hasFrontend and merge succeeds", async () => {
    let restartedBranch = "";
    const devServers = {
      devUrl: "http://localhost:3000",
      startAll: async () => {},
      stopAll: () => {},
      restartOnBranch: async (branch: string) => { restartedBranch = branch; },
    } as unknown as DevServerManager;
    ...
    await mergeAndVerify(
      [successForges[0]], ["EC-1"], ["/repo"], true,
      runner, devServers, makeJira(), makeTracker(), log,
    );

    assert.equal(restartedBranch, "my-merge-branch");
  });

  it("does not restart servers when hasFrontend is false", async () => {
    let restarted = false;
    const devServers = {
      devUrl: "http://localhost:3000",
      startAll: async () => {},
      stopAll: () => {},
      restartOnBranch: async () => { restarted = true; },
    } as unknown as DevServerManager;
  ...
  it("logs warning when jira moveTicket fails", async () => {
    const jira = {
      ticketUrl: () => "",
  ...
  it("still succeeds when verify fails", async () => {
    const runner = makeRunner([
      { code: 0, stdout: "branch" },
  ...
  it("still succeeds when PR creation fails", async () => {
    const runner = makeRunner([
      { code: 0, stdout: "branch" },
      { code: 0, stdout: "ok" },
...
describe("processLayers", () => {
  ...
  it("processes all layers and counts results", async () => {
  ...
  it("skips empty layers after filtering", async () => {
    const layers: GroupedLayer[] = [
      { group: ["EC-1"], relation: null, hasFrontend: false },
  ...
  it("respects skipped and excluded sets", async () => {
    const layers: GroupedLayer[] = [
  ...
  it("counts failures from forge errors", async () => {
    const layers: GroupedLayer[] = [
      { group: ["EC-1"], relation: null, hasFrontend: false },
    ];
  ...
  it("returns zeros for empty layers", async () => {
    const runner = makeFullRunner();
    const { log } = collectLogs();
  ...
  it("logs layer info with relation", async () => {
    const layers: GroupedLayer[] = [
      { group: ["EC-1"], relation: "same-epic", hasFrontend: false },
    ];
    const runner = makeFullRunner();
    const { logs, log } = collectLogs();
  ...
  it("logs layer info without relation when null", async () => {
    const layers: GroupedLayer[] = [
      { group: ["EC-1"], relation: null, hasFrontend: false },
    ];
    const runner = makeFullRunner();
