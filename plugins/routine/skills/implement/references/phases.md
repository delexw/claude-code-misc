...
> **Parallelism:** Phase 3 and Phase 4 both depend only on Phase 2 output — they do NOT depend on each other. **Launch all Phase 3 and Phase 4 sub-skills concurrently** in a single batch. Within each phase, individual skill calls are also independent and should run in parallel. Wait for all to complete before proceeding to Phase 5.

### Phase 3: domain_discover (via `Skill("domain-discover")`)

- Invoke `Skill("domain-discover")` for each domain listed in `<task><domains>` concurrently, passing:
  `{domain_name} {TICKET_ASSETS_DIR}/domains` (e.g. `payments .implement-assets/EC-10420/domains`)
