...
- Visit each link — **invoke all link visits in a single message** for parallelism:
   ...
   - Other links (`links.other`): use `WebFetch` — write output to `SKILL_DIR/references/intel/scraps/{index}.md`
- **Wait for all skill invocations to complete**, then read each output file:
   - Confluence: `SKILL_DIR/references/intel/scrolls/{index}/scroll.md`
   - Jira: `SKILL_DIR/references/intel/dossiers/{linked_key}/dossier.json`
...
- **If Figma links found:** For each link, invoke `Skill("figma-reader")` with the Figma link and `SKILL_DIR/references/blueprints/{index}` as the output directory (numeric index starting at 0). **Wait for all to complete**, then read each `SKILL_DIR/references/blueprints/{index}/blueprint.md`.
- **Only if NO Figma links found:** Check downloaded attachments from Phase 2 (`SKILL_DIR/references/`) for images that appear to be UI designs. Invoke `Skill("figma-reader")` with the image path and `SKILL_DIR/references/blueprints/0` as the output directory. **Wait for it to complete**, then read `SKILL_DIR/references/blueprints/0/blueprint.md`.
