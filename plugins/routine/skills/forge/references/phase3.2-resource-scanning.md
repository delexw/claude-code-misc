...
- **If Figma links found:** For each link, invoke `Skill("figma-reader")` with args `{figma_link} {SKILL_DIR}/specs/{index}` (no quotes — space-separated, numeric index starting at 0). **Wait for all to complete**, then read each `SKILL_DIR/specs/{index}/output.md`.
- **Only if NO Figma links found:** Check downloaded attachments from Phase 2 (`SKILL_DIR/jira/`) for images that appear to be UI designs. Invoke `Skill("figma-reader")` with args `{image_path} {SKILL_DIR}/specs/0` (no quotes). **Wait for it to complete**, then read `SKILL_DIR/specs/0/output.md`.
